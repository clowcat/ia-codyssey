...
## 1. 프로젝트 개요
[cite_start]본 프로젝트는 개발자로서 필수적인 리눅스 CLI, Docker, Git/GitHub의 기초를 다지고, "내 컴퓨터에서만 돌아가는 문제"를 해결하기 위한 재현 가능한 환경 구축을 목표 [cite: 5, 6]

## 2. 실행 환경 (Environment)
[cite_start]보안 정책을 고려하여 OrbStack을 통해 sudo 권한 없이 실습 진행 [cite: 17, 19]
- **OS**: macOS 15.7.4
- **Shell**: zsh 5.9
- **Docker**: 28.5.2
- **Git**: 2.53.0
...
## 3. 핵심 이론 정리
### 3.1. [cite_start]Docker Image & Container 
- **이미지**: 불변(Immutable)의 실행 설계도이며 레이어 기반으로 구성됩니다.
- **컨테이너**: 이미지 위에 읽기/쓰기가 가능한 레이어를 얹은 독립적인 실행 단위입니다.

### [cite_start]3.2. exec vs attach 
- `attach`: 메인 프로세스에 합류. 종료 시 컨테이너도 중지됨.
- `exec`: 새로운 프로세스 생성. 종료 시에도 컨테이너는 유지됨 (운영 중 점검에 용이).

## 4. 수행 항목 및 검증 로그

### 4.1. [cite_start]터미널 및 권한 관리 [cite: 74-79]
