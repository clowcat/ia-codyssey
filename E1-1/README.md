1. 프로젝트 개요
본 프로젝트는 개발자로서 갖춰야 할 기본적인 도구인 **터미널(CLI), Docker(컨테이너), Git/GitHub(버전 관리)**의 세팅 및 활용 능력을 배양하는 것을 목표로 합니다. 단순히 도구를 설치하는 것에 그치지 않고, 재현 가능한 실행 환경을 구축하고 데이터 영속성 및 협업 기반의 소스코드 관리 원칙을 직접 검증합니다.


# E1-1: 개발 워크스테이션 구축 및 Docker/Git 실습

## 1. 프로젝트 개요
본 프로젝트는 개발자로서 필수적인 리눅스 CLI, Docker, Git/GitHub의 기초를 다지고, "내 컴퓨터에서만 돌아가는 문제"를 해결하기 위한 재현 가능한 환경 구축을 목표

## 2. 실행 환경 (Environment)
보안 정책을 고려하여 OrbStack을 통해 sudo 권한 없이 실습 진행
- **OS**: macOS 15.7.4
- **Shell**: zsh 5.9
- **Docker**: 28.5.2
- **Git**: 2.53.0
...
## 3. 핵심 이론 정리
### 3.1. Docker Image & Container 
- **이미지**: 불변(Immutable)의 실행 설계도이며 레이어 기반으로 구성
- **컨테이너**: 이미지 위에 읽기/쓰기가 가능한 레이어를 얹은 독립적인 실행 단위

### 3.2. exec vs attach 
- `attach`: 메인 프로세스에 합류. 종료 시 컨테이너도 중지됨.
- `exec`: 새로운 프로세스 생성. 종료 시에도 컨테이너는 유지됨 (운영 중 점검에 용이).
...
```bash
root@47e93d9277ea:/# echo "Hello, container!" >sample.txt
root@47e93d9277ea:/# cat sample.txt
Hello, container!
```
...
```bash
cyanc01125000@c6r7s3 E1-1 % docker start my-ubuntu            
my-ubuntu
cyanc01125000@c6r7s3 E1-1 % docker exec -it my-ubuntu /bin/bash
root@dc5849236cd9:/# exit
exit
cyanc01125000@c6r7s3 E1-1 % docker ps -a
CONTAINER ID   IMAGE     COMMAND       CREATED          STATUS          PORTS     NAMES
dc5849236cd9   ubuntu    "/bin/bash"   11 minutes ago   Up 12 seconds             my-ubuntu
```

```bash
cyanc01125000@c6r7s3 E1-1 % docker attach my-ubuntu
root@dc5849236cd9:/# eixt
bash: eixt: command not found
root@dc5849236cd9:/# exit
exit
cyanc01125000@c6r7s3 E1-1 % docker ps -a
CONTAINER ID   IMAGE     COMMAND       CREATED          STATUS                       PORTS     NAMES
dc5849236cd9   ubuntu    "/bin/bash"   12 minutes ago   Exited (127) 3 seconds ago             my-ubuntu
```
...
```bash
cyanc01125000@c4r4s3 mission1 % docker volume create my-data
my-data
